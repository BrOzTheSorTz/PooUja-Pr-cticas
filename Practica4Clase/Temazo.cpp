...
/**
 * @brief Asignamos el valor a la variable de clase por defecto
 */
Fecha Temazo:: fechaUltima;
/**
 * @brief Constructor por defecto
 */
Temazo:: Temazo():fechaUltimoUso(fechaUltima){
...
:titulo(_titulo),interprete(_interprete), duracion(_duracion),nombreUltimoGarito(_nombreUltimoGarito),fechaUltimoUso(_fechaUltimoUso){
    ...
    if(_genero.length()<= 2 ){
        throw ParametroNoValido("Temazo.cpp", " Constructor parametrizado"," Longitud no valida",_genero.length());
    }else if(_titulo.length()<=2){
        throw ParametroNoValido("Temazo.cpp", " Constructor parametrizado"," Longitud no valida",_titulo.length());

    }else if(_duracion <0){
        throw ParametroNoValido("Temazo.cpp", " Constructor parametrizado","Duracion no valida",_duracion);
...
Temazo& Temazo::setFechaUltimoUso(const Fecha &f) {
    ...
    fechaUltima=f; ///< Refrescamos la fecha del ultimo temazo
    return *this;
}
...
/**
 * @brief Constructor parametrizado sin el parametro Fecha
 * @param _titulo
 * @param _interprete
 * @param _duracion
 * @param _nombreUltimoGarito
 * @post Crea un objeto y le asigna la fecha del ultimo temazo asignado
 */
Temazo::Temazo(std::string _titulo, std::string _interprete, int _duracion, std::string _nombreUltimoGarito)
:titulo(_titulo),interprete(_interprete),duracion(_duracion),nombreUltimoGarito(_nombreUltimoGarito){
    ...
    this->fechaUltimoUso = fechaUltima; // Si no se indica la flecha se asigna la asignada del ultimo temazo
