...
string Poder::toCSV ( ) const
{
   ...
   //Hemos añadido el método getCapacidadDestructiva
   //porque a la hora de mostrar un poderPsiquico
   //nos muestre su capacidad destructiva aplicándole la lucidez
...
/**
 * @brief Comparo dos poderes
 * @param p Poder a comparar
 * @return Si un poder tiene más capacidad destructiva que otro
 */
bool Poder::operator<(const Poder &p) {

    return this->getCapacidadDestructiva() < p.getCapacidadDestructiva();
}

/**
 * @brief Asiganamos a los atributos de un objeto los valores dados en
 *      formato CSV
 * @param cadena valores en formato CSV
 */
void Poder::fromCSV(std::string cadena) {
    std::stringstream aux;
    aux.str(cadena);
    std::getline(aux,_nombre,';');
    std::getline(aux,_descripcion,';');
    std::getline(aux,_afectaA,';');
    aux >> _capacidadDestructiva;
