         ...
         _nacionalidad ( orig._nacionalidad ), _numPoderes ( orig._numPoderes )
{
   for ( int i = 0; i < orig._numPoderes; i++ )
   {
      ...
      /*FIXME
       * No, ya que en el caso de tener un PoderPsquico, no se copiaría
       * el atributo lucidez
       *
       * Para solucionar esto, es necesario un método addPoderPsiquico, addPoderFisico ,etc..
       * */
...
{
    //std::cout<<"COMPROBAR:"<<cadena<<std::endl;
...
Mutante& Mutante::operator= ( const Mutante& orig )
{
   if ( this != &orig )
   {
      ...
      for ( int i = 0; i < _numPoderes; i++ )
      {
          /*FIXME Ocurre lo mismo que en el constructor de copia.En el caso de tener
           * un poder psiquico, no se copiaría el atributo de lucidez
          */
         _poderes[i] = new Poder ( *orig._poderes[i] );
      }
   }
...
/**
 * @brief Comparamos dos mutantes en función de su capacidad destructiva total
 * @param m Mutante a comparar
 * @return Si uno tiene mayor capacidad destructiva que el otro
 */
bool Mutante::operator<( Mutante &m) {
    return this->capacidadDestructivaTotal() <m.capacidadDestructivaTotal();
}
