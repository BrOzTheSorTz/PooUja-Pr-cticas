...
Droide* Piloto::setAyudadoPorDroide(Droide *droide) {
    if(droide->getAveriado()){
        throw std::invalid_argument("El droide asignado esta averiado :( !");
    }
...
Informe Piloto::generaInforme() {
    ...
    if(this->getDroide()!= nullptr && !(this->getDroide()->getAveriado()) ){
        aux << "ID de la nave: " << this->getNave()->getIdSF() << ".Id del droide: " << this->getDroide()->getIdD()
            << ".Incidencias ultima mision: " << this->getIncidenciasUltimaMision();
        datos = aux.str();
    }else{
        if(this->getNave() == nullptr){
            aux << "ID de la nave: " <<"No tiene ninguna nave asignada " << ".Id del droide: " <<"No tiene ningun droide asignada porque la suya esta averiada"
                << ".Incidencias ultima mision: " << this->getIncidenciasUltimaMision();

            datos=aux.str();

        }else{
            aux << "ID de la nave: " << this->getNave()->getIdSF() << ".Id del droide: " <<"No tiene ninguna nave asignada porque la suya esta averiada"
                << ".Incidencias ultima mision: " << this->getIncidenciasUltimaMision();
